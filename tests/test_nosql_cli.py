...
class TestNoSQLCli(unittest.TestCase):
    ...
    def _seed(self):
        ...
        # add-many
        self._run([
            "--collection", self.collection, "--path", self.db_path, "add-many",
    ...
    def test_init_creates_file(self):
        code, out, err = self._run(["--collection", self.collection, "--path", self.db_path, "init"])
    ...
    def test_query_count_numeric_parsing(self):
        self._seed()
        code, out, err = self._run([
            "--collection", self.collection, "--path", self.db_path,
            "query", "--field", "age", "--op", "gt", "--value", "29", "--count"
        ])
        self.assertEqual(code, 0, msg=err)
    ...
    def test_query_first_eq(self):
        ...
        code, out, err = self._run([
            "--collection", self.collection, "--path", self.db_path,
            "query", "--field", "name", "--op", "eq", "--value", "Neel", "--first"
        ])
        self.assertEqual(code, 0, msg=err)
        doc = json.loads(out)
    ...
    def test_query_in_and_nin(self):
        ...
        code, out, err = self._run([
            "--collection", self.collection, "--path", self.db_path,
            "query", "--field", "age", "--op", "in", "--value", "[25,40]", "--count"
        ])
        self.assertEqual(code, 0, msg=err)
        ...
        code, out, err = self._run([
            "--collection", self.collection, "--path", self.db_path,
            "query", "--field", "age", "--op", "nin", "--value", "[25,40]", "--count"
        ])
        self.assertEqual(code, 0, msg=err)
    ...
    def test_query_matches_regex(self):
        ...
        code, out, err = self._run([
            "--collection", self.collection, "--path", self.db_path,
            "query", "--field", "name", "--op", "matches", "--value", "^(B|C)", "--count"
        ])
        self.assertEqual(code, 0, msg=err)
        self.assertEqual(int(out.strip()), 2)
    ...
    def test_query_exists_nested(self):
        ...
        code, out, err = self._run([
            "--collection", self.collection, "--path", self.db_path,
            "query", "--field", "address.city", "--op", "exists", "--count"
        ])
        self.assertEqual(code, 0, msg=err)
        self.assertEqual(int(out.strip()), 1)
    ...
    def test_query_projection_and_outfile(self):
        ...
        code, out, err = self._run([
            "--collection", self.collection, "--path", self.db_path,
            "query", "--field", "age", "--op", "gte", "--value", "25",
            "--fields", "id", "name", "--out", out_file
        ])
        self.assertEqual(code, 0, msg=err)
    ...
    def test_query_pretty_flag_changes_format(self):
        ...
        code, out_pretty, err = self._run([
            "--collection", self.collection, "--path", self.db_path,
            "query", "--field", "age", "--op", "gte", "--value", "25", "--pretty"
        ])
    ...
    def test_agg_count_sum_avg_min_max(self):
        self._seed()
        # count
        code, out, err = self._run(["--collection", self.collection, "--path", self.db_path, "agg", "count"])
        self.assertEqual(code, 0, msg=err)
        self.assertEqual(int(out.strip()), 3)
        # sum
        code, out, err = self._run(["--collection", self.collection, "--path", self.db_path, "agg", "sum", "--field", "age"])
        self.assertEqual(code, 0, msg=err)
        self.assertEqual(int(float(out.strip())), 95)
        # avg
        code, out, err = self._run(["--collection", self.collection, "--path", self.db_path, "agg", "avg", "--field", "age"])
        self.assertEqual(code, 0, msg=err)
        self.assertTrue(re.match(r"3[0-9]\.\d+", out.strip()))  # ~31.6
        # min
        code, out, err = self._run(["--collection", self.collection, "--path", self.db_path, "agg", "min", "--field", "age"])
        self.assertEqual(code, 0, msg=err)
        self.assertEqual(int(out.strip()), 25)
        # max
        code, out, err = self._run(["--collection", self.collection, "--path", self.db_path, "agg", "max", "--field", "age"])
        self.assertEqual(code, 0, msg=err)
        self.assertEqual(int(out.strip()), 40)

    def test_clear_then_count_zero(self):
        self._seed()
        code, out, err = self._run(["--collection", self.collection, "--path", self.db_path, "clear"])
        self.assertEqual(code, 0, msg=err)
        res = json.loads(out)
    ...
    def test_reject_in_memory_path(self):
        code, out, err = self._run(["--collection", self.collection, "--path", ":memory:", "init"], expect_exit=True)
    ...
    def test_reject_wrong_extension(self):
        ...
        code, out, err = self._run(["--collection", self.collection, "--path", bad_path, "init"], expect_exit=True)
        self.assertNotEqual(code, 0)
    ...
    def test_add_many_requires_array(self):
        self._run(["--collection", self.collection, "--path", self.db_path, "init"])
        code, out, err = self._run(
            ["--collection", self.collection, "--path", self.db_path, "add-many", '{"id":1}'],
            expect_exit=True
        )
        self.assertNotEqual(code, 0)
    ...
    def test_query_value_provided_for_exists(self):
        ...
        code, out, err = self._run([
            "--collection", self.collection, "--path", self.db_path,
            "query", "--field", "age", "--op", "exists", "--value", "1", "--count"
        ], expect_exit=True)
        self.assertNotEqual(code, 0)
    ...
    def test_in_nin_value_must_be_array(self):
        ...
        code, out, err = self._run([
            "--collection", self.collection, "--path", self.db_path,
            "query", "--field", "age", "--op", "in", "--value", "30", "--count"
        ], expect_exit=True)
        self.assertNotEqual(code, 0)
    ...
    def test_query_out_creates_parent_dir(self):
        ...
        code, out, err = self._run([
            "--collection", self.collection, "--path", self.db_path,
            "query", "--field", "age", "--op", "gte", "--value", "25", "--out", out_file
        ])
        self.assertEqual(code, 0, msg=err)
        self.assertTrue(os.path.exists(out_file))
    ...
    def test_query_unsupported_op(self):
        ...
        code, out, err = self._run([
            "--collection", self.collection, "--path", self.db_path,
            "query", "--field", "age", "--op", "weirdop", "--value", "1", "--count"
        ], expect_exit=True)
        self.assertNotEqual(code, 0)
        self.assertIn("unsupported operator", err.lower())


print("Running NoSQL CLI tests...")
