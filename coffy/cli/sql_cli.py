...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--db", required=True, help="Path to SQLite file, must be a file, not in-memory")
    ...
    sp_run.add_argument("--json", action="store_true", help="Emit SELECT results as JSON to stdout")
    sp_run.add_argument("--pretty", action="store_true", help="Pretty-print JSON when using --json")
    sp_run.add_argument("--out", help="If last statement is SELECT, export to .json or .csv")
    ...
    sp_export.add_argument("--out", required=True, help="Output path ending in .json or .csv")
    sp_export.set_defaults(func=cmd_export)

    sp_view = sub.add_parser("view", help="Run a SELECT and open HTML viewer in browser")
