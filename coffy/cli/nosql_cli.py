...
def cmd_query(args: argparse.Namespace) -> int:
    ...
    if op in {"in", "nin"}:
        ...
        val = _load_json_arg(args.value) if args.value.startswith(("@", "-")) else json.loads(args.value)
    ...
    if args.count:
        ...
        if args.out:
            ...
            print(json.dumps(docs, indent=2 if args.pretty else None, ensure_ascii=False))
    return OK
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--path", required=True, help="Path to JSON file backing the collection")
    ...
    sp_add = sub.add_parser("add", help="Add one document (JSON string, @file, or - for stdin)")
    sp_add.add_argument("document", help='JSON document, e.g. {"id":1} or @doc.json or -')
    sp_add.set_defaults(func=cmd_add)

    sp_add_many = sub.add_parser("add-many", help="Add many documents (JSON array string, @file, or -)")
    sp_add_many.add_argument("documents", help='JSON array, e.g. [{"id":2}] or @docs.json or -')
    sp_add_many.set_defaults(func=cmd_add_many)

    sp_query = sub.add_parser("query", help="Simple query on one field")
    sp_query.add_argument("--field", required=True, help="Field name, dot notation supported")
    sp_query.add_argument(
        "--op",
        required=True,
        help="Operator: eq, ne, gt, gte, lt, lte, in, nin, exists, matches",
    )
    sp_query.add_argument("--value", help="Value for operator. JSON array for in or nin")
    ...
    sp_query.add_argument("--pretty", action="store_true", help="Pretty-print JSON to stdout")
    sp_query.set_defaults(func=cmd_query)

    sp_agg = sub.add_parser("agg", help="Collection-level aggregation")
    sp_agg.add_argument("agg", choices=["sum", "avg", "min", "max", "count"], help="Aggregation function")
